/**
 * Map Tab
 * Home / Live Heatmap (Dark Theme)
 * 
 * Full-bleed map, floating search bar, color-coded pins, bottom-center Scan FAB.
 */
...
export default function MapScreen() {
  ...
  const [targetDay, setTargetDay] = useState(new Date().getDay()); // 0 (Sun) to 6 (Sat)
  const [targetHour, setTargetHour] = useState(new Date().getHours()); // 0 to 23
  ...
  const { data: logs, isLoading: isLoadingLogs } = useNoiseLogs(null, {
    isPredictive,
    targetDay,
    targetHour
  });
  ...
  const handleTogglePredictive = () => {
    if (!entitlements?.is_pro) {
      Alert.alert(
        'QuietZone Pro',
        'Predictive Filtering is a Pro feature. Upgrade to see historical noise predictions for any day and time!',
        [
          { text: 'Maybe Later', style: 'cancel' },
          { text: 'Upgrade', onPress: () => router.push('/profile') } // Assuming profile has upgrade
        ]
      );
      return;
    }
  ...
  return (
    <View style={[styles.container, isExpanded && { flexDirection: 'row' }]}>
      {/* Expanded Split Panel: Left List */}
      {isExpanded && (
        <View style={styles.sidePanel}>
          <DiscoverScreen isNested={true} />
        </View>
      )}
      ...
      <View style={styles.mapContainer}>
        ...
        {/* Floating Header Area - Hidden on Expanded because side panel handles search */}
        {!isExpanded && (
        <View style={[styles.floatingHeader, { top: Math.max(insets.top, Spacing.md) }]}>
          <View style={styles.topRow}>
            ...
            <TouchableOpacity 
              style={[styles.toggleButton, isPredictive && styles.toggleButtonActive]} 
              onPress={handleTogglePredictive}
            >
              <Ionicons 
                name={isPredictive ? "analytics" : "time-outline"} 
                size={20} 
                color={isPredictive ? DarkColors.liveAccent : DarkColors.textSecondary} 
              />
            </TouchableOpacity>
          </View>

          {/* Predictive Time Picker UI */}
          {isPredictive && (
          <View style={styles.pickerPanel}>
            ...
            <View style={styles.pickerControls}>
              {/* Day Cycler */}
              <TouchableOpacity style={styles.pickerBtn} onPress={() => setTargetDay((d) => (d === 0 ? 6 : d - 1))}>
                <Ionicons name="chevron-back" size={16} color={DarkColors.textPrimary} />
              </TouchableOpacity>
              <Text style={styles.pickerValue}>{DAYS[targetDay]}</Text>
              <TouchableOpacity style={styles.pickerBtn} onPress={() => setTargetDay((d) => (d === 6 ? 0 : d + 1))}>
                <Ionicons name="chevron-forward" size={16} color={DarkColors.textPrimary} />
              </TouchableOpacity>

              <View style={styles.pickerDivider} />

              {/* Hour Cycler */}
              <TouchableOpacity style={styles.pickerBtn} onPress={() => setTargetHour((h) => (h === 0 ? 23 : h - 1))}>
                <Ionicons name="chevron-back" size={16} color={DarkColors.textPrimary} />
              </TouchableOpacity>
              <Text style={styles.pickerValue}>{targetHour}:00</Text>
              <TouchableOpacity style={styles.pickerBtn} onPress={() => setTargetHour((h) => (h === 23 ? 0 : h + 1))}>
                <Ionicons name="chevron-forward" size={16} color={DarkColors.textPrimary} />
              </TouchableOpacity>
            </View>
          </View>
      ...
      {!isExpanded && !isPredictive && (
        <TouchableOpacity 
          style={[styles.fab, { bottom: insets.bottom + Spacing.xl + 60 }]}
          activeOpacity={0.9}
