...
export default function DiscoverScreen({ isNested = false }) {
  ...
  const { data: noiseLogs, isLoading, isError } = useNoiseLogs(null, {
    search: searchQuery,
  ...
  // Combine and sort businesses first, then noise logs
  const combinedData = React.useMemo(() => {
    let result = [];
    if (businesses) {
      // Filter businesses by search text roughly
      const bizFiltered = businesses.filter(b => 
        !searchQuery || b.name.toLowerCase().includes(searchQuery.toLowerCase())
      );
      result = [...bizFiltered.map(b => ({ ...b, type: 'business' }))];
    }
    if (noiseLogs) {
      result = [...result, ...noiseLogs.map(n => ({ ...n, type: 'log' }))];
    }
    return result;
    ...
    <View style={styles.headerContainer}>
      ...
      <View style={styles.resultCountContainer}>
        <Text style={styles.resultCountText}>
          {combinedData.length} {combinedData.length === 1 ? 'place' : 'places'} found
        </Text>
      </View>
    </View>
  );
  ...
  const renderEmpty = () => {
    if (isLoading || isLoadingBiz) {
      return (
        <View style={styles.emptyContainer}>
          <ActivityIndicator size="large" color={LightColors.brandPrimary} />
        </View>
      );
    }
    ...
    if (isError) {
      return (
        <View style={styles.emptyContainer}>
          <Ionicons name="alert-circle-outline" size={48} color={LightColors.textSecondary} />
          <Text style={styles.emptyTitle}>Failed to load scans</Text>
    ...
    return (
      <View style={styles.emptyContainer}>
        <Ionicons name="search-outline" size={48} color={LightColors.textSecondary} />
  ...
  const renderItem = ({ item }) => {
    ...
    if (item.type === 'business') {
      return (
        ...
        onPress={() => {
            router.push({
              pathname: '/map',
              params: { targetLat: item.latitude, targetLng: item.longitude }
            });
          }}
        />
      );
    ...
    return (
      <ListItemCard
        ...
        meta={timeAgo}
        leftContent={
          <DisruptionScoreBadge score={item.disruption_score} />
        }
        onPress={() => {
          router.push({
            pathname: '/map',
